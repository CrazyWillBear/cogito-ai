...
def agent_assigner(messages: list[str]):
    ...
    prompt = f"You are a router that determines whether the last message sent by the user/human would benefit from \
              the AI to perform deep research to inform the response. Respond with ONLY 'Yes' (if it requires research) \
              or 'No' (if it doesn't). Here are the messages: {messages}"
