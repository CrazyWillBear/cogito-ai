...
def _extract_sections(url):
    """Extract all sections from a SEP article with their headers and content."""
    headers = {"User-Agent": "Cogito Research Bot (wbc008@bucknell.edu)"}
...
def _process_article(result, last_user_msg):
    """Process a single article: extract sections, select relevant ones, return list of (text, citation) tuples."""
    sections, base_citation = _extract_sections(result["url"])

    if not sections:
        return []

    # Use LLM to select relevant sections
    relevant_sections = _select_relevant_sections(sections, last_user_msg, result["title"])
...
def query_sep(queries: list[str], last_user_msg: str) -> list[QueryResult]:
    ...
    # Build a list of (query, search_result) jobs so we know which query
    # each SEP article came from.
    ...
    # Process articles in parallel, carrying the originating query along
    with ThreadPoolExecutor(max_workers=3) as executor:
        future_to_query = {
            executor.submit(_process_article, result, last_user_msg): query
        ...
        for future in as_completed(future_to_query):
            ...
            try:
                article_tuples = future.result()
            except Exception as e:
                print(f"Error processing article for query '{query}': {e}")
                continue
